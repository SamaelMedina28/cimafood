      ...
      <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="p-6 sm:p-8">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            {{-- Imagen --}}
            <div class="flex items-center justify-center bg-gray-50 rounded-3xl border border-gray-100 overflow-hidden min-h-[300px]">
            ...
            <div class="flex flex-col">
              <div class="mb-6">
                ...
                <p class="text-4xl font-bold text-emerald-600">${{ number_format($product->price, 2) }}</p>
              </div>

              <div class="space-y-6 flex-1">
                <div>
                  <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Descripción</h4>
                  <p class="text-gray-700 leading-relaxed text-lg">{{ $product->description ?: 'No hay descripción disponible para este producto.' }}</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                    <div>
                        ...
                        @if (isset($product->status) && $product->status === 'available')
                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-emerald-50 text-emerald-700 border border-emerald-200/60">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-2"></span>
                                Disponible
                            </span>
                        @elseif(isset($product->status) && $product->status !== 'available')
                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-gray-50 text-gray-600 border border-gray-200">
                    ...
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Inventario</h4>
                        <p class="text-gray-900 font-medium text-lg flex items-center">
                            @if ($product->quantity !== null)
                                <svg class="w-5 h-5 mr-2 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                </svg>
                                {{ $product->quantity }} <span class="text-gray-500 text-sm ml-1">unidades</span>
                            @else
                    ...
                    <div class="col-span-2">
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Negocio Asociado</h4>
                        <div class="flex items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                            @if ($product->business)
                                <div class="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center mr-3 shadow-sm">
                                ...
                                <span class="text-gray-900 font-medium text-base">{{ $product->business->name }}</span>
                            @else
