        ...
        <div class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
            ...
            @if ($businesses->count() > 1)
                <div class="w-full lg:w-48">
                    <select
                        wire:model.live="businessFilter"
    ...
    <div class="bg-white overflow-hidden">
        @if ($orders->count() === 0)
            <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
                <div class="w-16 h-16 bg-gray-50 rounded-lg flex items-center justify-center mb-4 border border-gray-100">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr class="border-b border-gray-200 bg-gray-50/70">
                            ...
                            @if ($businesses->count() > 1)
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Negocio</th>
                            @endif
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Productos</th>
                    ...
                    <tbody class="divide-y divide-gray-150 bg-white">
                        @foreach ($orders as $order)
                            @php
                                $statusConfig = [
                                    'pending'   => ['label' => 'Pendiente',  'class' => 'bg-amber-50 text-amber-700 border-amber-200/60',      'dot' => 'bg-amber-500'],
                                    'completed' => ['label' => 'Completado', 'class' => 'bg-emerald-50 text-emerald-700 border-emerald-200/60','dot' => 'bg-emerald-500'],
                                    'cancelled' => ['label' => 'Cancelado',  'class' => 'bg-red-50 text-red-600 border-red-200/60',            'dot' => 'bg-red-500'],
                                ];
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0 shadow-sm border border-gray-300">
                                            <span class="text-xs font-bold text-gray-600">{{ strtoupper(substr($order->user->name ?? 'U', 0, 1)) }}</span>
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-gray-900 truncate">{{ $order->user->name ?? 'Usuario' }}</p>
                                            <p class="text-xs text-gray-500 truncate hidden sm:block">{{ $order->user->email ?? '' }}</p>
                                        </div>
                                    </div>
                                </td>
                                @if ($businesses->count() > 1)
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    <span class="text-xs text-gray-500">
                                        {{ $order->products->count() }} {{ $order->products->count() === 1 ? 'producto' : 'productos' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm font-bold text-gray-950">${{ number_format($order->total_price, 2) }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $cfg['class'] }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ $cfg['dot'] }}"></span>
                                        {{ $cfg['label'] }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 hidden lg:table-cell">
                                    <p class="text-xs font-medium text-gray-700">{{ $order->created_at->format('d/m/Y') }}</p>
            ...
            @if ($selectedOrder)
                <div class="space-y-5">
                    ...
                    <div class="p-4 bg-gray-50 border border-gray-200 rounded-md">
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Información del Cliente</h4>
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0 border border-gray-300 shadow-sm">
                                <span class="text-xs font-bold text-gray-600">{{ strtoupper(substr($selectedOrder->user->name ?? 'U', 0, 1)) }}</span>
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-semibold text-gray-900">{{ $selectedOrder->user->name ?? 'Usuario eliminado' }}</p>
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200 flex justify-between text-xs text-gray-500">
                            <div>
                                <span class="font-semibold text-gray-700">Fecha:</span> {{ $selectedOrder->created_at->format('d/m/Y h:i A') }}
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Productos --}}
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Productos del Pedido</h4>
                        <div class="space-y-2 max-h-52 overflow-y-auto pr-1">
                            @foreach ($selectedOrder->products as $product)
                                <div class="flex items-center justify-between py-2 px-3 bg-gray-50 border border-gray-200 rounded-md">
                                    <div class="min-w-0">
                                        <p class="text-xs font-semibold text-gray-900 truncate">{{ $product->name }}</p>
                                        <p class="text-2xs text-gray-500">
                                            x{{ $product->pivot->quantity }} · ${{ number_format($product->pivot->price_unit, 2) }} c/u
                                        </p>
                                    </div>
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-gray-200">
                        ...
                        <span class="text-lg font-bold text-gray-950">${{ number_format($selectedOrder->total_price, 2) }}</span>
                    </div>
                </div>
            @endif
        </x-slot>
